...
bubble_plot <- function(df, x_value, y_value, freq, colour_choice, title){
  ...
  geom_point(aes(size = {{freq}},  # bubbles with the size according to how many respondents
                 fill = {{x_value}}), # bubble color according to x value level
             alpha = 0.75, 
             shape = 19, color=NA) + # circles and no border
  scale_size(range = c(1.4, 10)) + # size of the bubbles
...
bubble<-bubble_plot(df = relation_ordinal_Q22_25, x_value = overtime_work_25, y_value = job_satisfaction_22, freq = Freq, colour_choice = colours, title = "") + bubble_plotly_theme()
  
ggplotly(bubble, tooltip="text", width = 800, height = 600)
```
