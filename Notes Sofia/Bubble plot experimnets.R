...
ggplot(heatmap_Q22_Q25, aes(x = overtime_work_25, y = job_satisfaction_22)) + 
  geom_point(aes(size = Freq, 
                 fill = overtime_work_25), 
             alpha = 0.75, 
             shape = 21) + 
  scale_size_continuous(limits = c(0.0000001, 50), 
                        range = c(1,17), 
                        breaks = c(1,10,50,75)) + 
  labs( x= "", y = "", size = "Number of respondents", fill = "")  + 
  theme(legend.key=element_blank(), 
        axis.text.x = element_text(colour = "black", size = 12, face = "bold", family="Gotham"), 
        axis.text.y = element_text(colour = "black", face = "bold", size = 11), 
        legend.title = element_text(size = 12, face = "bold"), 
        panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2), 
        legend.position = "right") +  
  scale_fill_manual(values = colours, guide = FALSE) + 
  scale_y_discrete(limits = rev(levels(heatmap_Q22_Q25$job_satisfaction_22))) 
...
p <- heatmap_Q22_Q25 %>%
  ...
  ggplot( aes(x=overtime_work_25, y=job_satisfaction_22, 
              text=text)) +
  theme_minimal() +
  geom_point(aes(size = Freq, 
                 fill = overtime_work_25), 
